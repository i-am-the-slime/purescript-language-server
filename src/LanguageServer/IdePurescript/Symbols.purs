...
getDefinition docs _ (ServerState { pscIdePort, modules, root }) ({ textDocument, position }) = do
  ...
  case maybeDoc of
    ...
    Just doc -> do
      ...
      case pscIdePort, root, identifierAtPoint text (_.character $ un Position position) of
        Just port, Just workspaceRoot, Just { word, qualifier } -> do
          getDefinitionFromPscIde port workspaceRoot qualifier modules word
...
getDefinitionFromPscIde ∷ Int -> String -> Maybe String -> Modules.State -> String -> Aff (Nullable Location)
getDefinitionFromPscIde pscIdePort workspaceRoot qualifier modules word = do
  let unqualifiedActiveModules = getUnqualActiveModules modules (Just word)
  let getQualifiedModule = flip getQualModule modules
  info <- getTypeInfo pscIdePort word modules.main qualifier unqualifiedActiveModules getQualifiedModule
  liftEffect $ toNullable
    <$> case info of
        Just (Command.TypeInfo { definedAt: Just (Command.TypePosition { name, start }) }) -> do
          uri <- filenameToUri =<< resolve [ workspaceRoot ] name
    ...
    let
      kind =
        if Str.take 1 identifier == (Str.toUpper $ Str.take 1 identifier) then
          ...
          if contains (Pattern "->") identifier then
            FunctionSymbolKind
          else
            PropertySymbolKind
